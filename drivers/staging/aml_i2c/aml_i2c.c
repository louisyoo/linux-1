...
{	
	...
	//struct clk *sys_clk;
	aml_i2c_reg_ctrl ctrl;

	//sys_clk = clk_get_sys("clk81", NULL);
	//sys_clk_rate = clk_get_rate(sys_clk);
	sys_clk_rate = 200000000;/*not implement yet*/

	i2c_clock_set = sys_clk_rate / i2c->master_i2c_speed;
	i2c_clock_set >>= 2;

	ctrl.word = ioread32(&(i2c->master_regs->i2c_ctrl));
	ctrl.bits.clk_delay = i2c_clock_set & AML_I2C_CTRL_CLK_DELAY_MASK;
	iowrite32(ctrl.word, &(i2c->master_regs->i2c_ctrl));
}
...
static void aml_i2c_pinmux_master(struct aml_i2c *i2c)
{
	...
	scl_pinmux = ioread32(i2c->master_pinmux.scl_reg);
	scl_pinmux |= i2c->master_pinmux.scl_bit;
	iowrite32(scl_pinmux, i2c->master_pinmux.scl_reg);
	
	sda_pinmux = readl(i2c->master_pinmux.sda_reg);
	sda_pinmux |= i2c->master_pinmux.sda_bit;
	iowrite32(sda_pinmux, i2c->master_pinmux.sda_reg);
}
...
/*poll status*/
//static 
int aml_i2c_wait_ack(struct aml_i2c *i2c)

{
	...
	for(i=0; i<i2c->wait_count; i++) {
		...
		ctrl.word = ioread32 (&(i2c->master_regs->i2c_ctrl));
		if(ctrl.bits.status == IDLE)
			return aml_i2c_check_error(i2c);
		
                /*cause BUG: scheduling while atomic?*/
		/*cond_resched();*/
	}
							...
							int num)
{
	...
	AML_I2C_DBG("aml_blai2c_xfer_slow");
	if (p->flags & I2C_M_RD){
		AML_I2C_DBG("read ");
	}
	else {
	...
	/* Return the number of messages processed, or the error code*/
	if (ret == 0) {
		printk (KERN_ALERT "returning num =%d\n", num);
		return num;
	} else {
		printk("[aml_i2c_xfer] error ret = %d \t", ret);
		printk(KERN_ALERT "i2c master %s current slave addr is 0x%x \t", 
			i2c->master_no?"a":"b", i2c->cur_slave_addr);
		return ret;
	}
